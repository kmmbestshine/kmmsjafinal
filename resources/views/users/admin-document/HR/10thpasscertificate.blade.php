...
<hr>
    <h3 style="text-align: center;"><u>10th CLASS PASSING  CERTIFICATE</u></h3>
    ...
    <div class="content"> <!-- use this div only if it is required for styling -->
        <p>
         &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspThis is to certify that Thiru / selvi {{$student->student_name}} S/o / D/o {{$student->parent_name}} residing at {{$student->address}} was a bonafide student of our school studied from {{$fromclass->class}} standard to {{$toclass->class}} standard from the academic year {{$input['fromyear']}} to {{$input['toyear']}}
        </p>
        <p>
        	&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspHe / She has successfully completed the qualifying examination held during {{$session->session}} for X STANDARD conducted by TAMIL NADU STATE BOARD and passed with {{$input['marks']}}% of marks
        </p>
      </div>
		<div class="row">
